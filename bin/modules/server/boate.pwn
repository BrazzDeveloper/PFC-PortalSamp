...
new iBitchLabel[BITCHES][128] = {
    "Emme White: {FFFFFF}Estou tão molhadinha, vem tirar minha calcinha.. Vem cá querido!\nPressione F para Sexo\n{"COR_VERDE_INC"}R$10.000",
    "Pamela Santos: {FFFFFF}Eai bebe, tá afim de um joguinho?\nPressione F para um Boquete\n{"COR_VERDE_INC"}R$4.500",
    "Dread Hot: {FFFFFF}Estou afim de um leitinho amor, me dá?\nPressione F para um Boquete\n{"COR_VERDE_INC"}R$3.500",
    "Tigresa VIP: {FFFFFF}Estou te esperando, vem encher minha buceta de leitinho..\nPressione F para Sexo\n{"COR_VERDE_INC"}R$15.000"
};
...
public Excitement(playerid, bitchid)
{
    ...
    if ( Player[playerid][excMod] == 0) {
        ...
        if((bitchid==0) || (bitchid==3)) {
            TextDrawSetString(gagnfuck,"Pressione LMB e RMB ao mesmo tempo para comer ela");
        } else {
            TextDrawSetString(gagnfuck,"Pressione LMB e RMB ao mesmo tempo para ela lhe chupar");
        ...
        if(Player[playerid][pExcitement] > 0) { 
            Player[playerid][pExcitement] -= 3.0; 
        }
        if(Player[playerid][pExcitement] < 86.0) {
