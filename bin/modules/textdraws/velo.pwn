...
new bool:gpIsSpeedoVisible[MAX_PLAYERS];
...
stock ShowPlayerSpeedo(playerid, vehicleid)
{
	if(!gpIsSpeedoVisible[playerid])
	{
		...
		PlayerTextDrawSetPreviewVehCol(playerid, Speedo[playerid][2], 1, 1);
		...
		new Float:fGasPercentage = floatdiv(call::VH->GetVehicleFuel(carro), 10.0);
		fGasPercentage *= 0.53;
		...
		new sGas[9];
		format(sGas, sizeof(sGas), "%.0f%%", call::VH->GetVehicleFuel(vehicleid));
		...
		Speedo[playerid][23] = CreatePlayerTextDraw(playerid, 583.983215, 414.400024, "as");
...
ptask UpdatePlayerSpeedo[100](playerid)
{
	...
	new	Float:health, Float:speed = GetVehicleSpeeed(playerid), vehicleid = GetPlayerVehicleID(playerid);
	GetVehicleHealth(vehicleid, health);
	...
	PlayerTextDrawColor(playerid, Speedo[playerid][19], (speed > 65) ? 0xFF0000FF : 0xFFFFFFFF);
	...
	tenH = call::VH->GetVehicleFuel(vehicleid);
	red = 255;
	green = 0;
	if(call::VH->GetVehicleFuel(vehicleid) > 50) {
		red -= floatround(2.55 * (tenH - 50));
		green = 255;
	} else {
    	green += floatround(5.1 * tenH);
	...
	new sGas[9];
	format(sGas, sizeof(sGas), "%.0f%% litros", call::VH->GetVehicleFuel(vehicleid));
	...
	new Float:fGasPercentage = call::VH->GetVehicleFuel(vehicleid);
	fGasPercentage *= 0.53;
	fGasPercentage += 498.487945;
