...
*
*/
// discord_bot_token ODMxNTc2OTYxMDEyODU4OTMx.YHXQZQ.uszbAW3mOqiJWq2XgWRvftpuhL4
...
#define SERVER_VERSION 							"1.2.5.1"
#define VERSION_SAMP_REQUIRED 					"0.3.7-R2"
#define SERVER_SITE 							"perfectcityrpg.com.br"
#define SERVER_EMAIL 							"suporte@perfectcityrpg.com.br"
#define minutos(%0) 							(1000 * %0 * 60)

new const CMD_NOT_PERMISSION[] = 				"Você não tem permissão para usar este comando.";
#define CMD_CORRECT_USE							"| ERRO | Uso Correto: %s"

#define MIN_XP									5
#define MAX_TOPS 								10

#define VALOR_POR_ESTRELAS 						1000
...
#define COR_ERRO								0xE34949FF
#define COR_ERRO_INC 							"E34949"
...
// ============================== [ DEFINIÇÃO DE CHAMADA PERSONALIZADA ] ============================== //
...
new const MotivosSu[][e_CrimesInfo] =
{
	...
	{ "Roubo de Veículos", "RV", 4, "20 min cadeia = 5 por estrela" },
...
new 
	CP_AGENCIA_1, CP_AGENCIA_2, CP_POUPA_TEMPO[3], CP_BAYSIDE, CP_PREFEITURA, CP_DETRAN[7], CP_BANCO[6], CP_ARMAS, CP_DP[3], CP_ARMASDP[3], CP_HOSPITAL, bool:Blocked[MAX_PLAYERS];
...
hook OnGameModeInit()
{
	...
	SendRconCommand("gamemodetext PFC:RPG ("SERVER_VERSION")");
	SendRconCommand("language Português Brasil");
	SendRconCommand("password sadhusahdauhs");
	SetTimer("RemoverSenhaInicio", 10000, false);

	// Status 
	SetTimer("Timer@UP", 50000, true);
	SetTimer("Timer@SEDE", 120000, true); //120000
	SetTimer("Timer@FOME", 300000, true); //300000
	SetTimer("Timer@SONO", 540000, true); //540000
	...
	CP_PREFEITURA = CreateDynamicCP(1476.1644,-1780.7090,15.4440, 1.1, .streamdistance=20.0, .interiorid=0, .worldid=-1);
	CreateDynamic3DTextLabel("Prefeitura", 0x14B27CFF, 1476.1644,-1780.7090,15.4440, 10.0, .streamdistance=20.0, .interiorid=0);


	//ActorSpawn = CreateActor(217, 1688.1864,-2246.5581,13.5469,30.4298);
	CreateActor(217, 1688.1864,-2246.5581,13.5469,30.4298);
	CreateDynamic3DTextLabel("Informações Úteis\nDigite {"COR_VERDE_CLARO_INC"}/infos", -1, 1688.1864,-2246.5581,13.5469, 10.0, .streamdistance=20.0, .interiorid=0);
...
public RemoverSenhaInicio()
{
	...
	SendRconCommand("gamemodetext PFC:RPG ("SERVER_VERSION")");
	SendRconCommand("language Português Brasil");
	SendRconCommand("password Acesso@pfc");
	return 1;
}
...
public OnPlayerCommandReceived(playerid, cmd[], params[], flags)
{
	if( !IsPlayerAdmin(playerid) )
	{
		...
		if((flags & DIRECAO) && call::ADMIN->GetPlayerAdminLevel(playerid) < flags ||
			...
			return SendClientMessage(playerid, COR_ERRO, CMD_NOT_PERMISSION), false;
		...
		if(call::ADMIN->GetPlayerAdminLevel(playerid) < DIRECAO && (flags & AJUDANTE || flags & MODERADOR || flags & ADMINISTRADOR) && !call::ADMIN->IsAdminInJob(playerid))
			return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você precisa usar /trabalharadm para utilizar comandos administrativos."),false;
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
	...
	else if(checkpointid == CP_AGENCIA_1 || checkpointid == CP_AGENCIA_2)
	{
		...
		ShowPlayerDialog(playerid, MENU_DP, DIALOG_STYLE_TABLIST, "MENU DP", "Limpar Ficha\t{"COR_VERDE_INC"}R$%s\nPresos", "Selecionar", "Fechar", RealStr(valor));
	}
	else if ( checkpointid == CP_ARMASDP[0] || checkpointid == CP_ARMASDP[1] || checkpointid == CP_ARMASDP[2] )
	{
	    ...
	    if(Blocked[playerid] == true) return SendClientMessage(playerid, COR_ERRO, "| ERRO | Vocï¿½ jï¿½ se equipou recentemente.");
...
Dialog:CONFIMAR_VENDA_VEICULO(playerid, response, listitem, inputtext[])
{
	...
	if ( response )
	{
		...
		if( Vehicle[vehicleid][Owner][Id] != Jogador[playerid][PlayerID] )
			return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você não é proprietário desse veiculo.");
	...
	if ( response )
	{
		switch(listitem)
		{
			case 0: // Planos
			{
				...
				ShowPlayerDialog(playerid, MENU_PLANOS_SAUDE, DIALOG_STYLE_TABLIST, "PLANOS DE SAï¿½DE", str, "Contratar", "Voltar");
				return true;
			}
			case 1:
			{
				...
				if ( health >= 100 )
					return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você está com a saúde ótima, volte quando estiver doente.");
				...
				SendClientMessage(playerid, COR_AZUL, "* {"COR_BRANCO_INC"}Você está recebendo o devido tratamento da equipe médica. Por favor aguarde...");
				return true;
			}
		}
		return true;
...
Dialog:MENU_PLANOS_SAUDE(playerid, response, listitem, inputtext[])
{
	if ( response )
	{
		if ( call::PLAYER->GetPlayerVarInt(playerid, Plano) == listitem )
			return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você já é assinante desse plano de saúde."), OnPlayerEnterDynamicCP(playerid, CP_HOSPITAL);
		...
		SendClientMessage(playerid, COR_AZUL, "* {"COR_BRANCO_INC"}Você assinou o plano de saúde {"COR_AZUL_INC"}%s{"COR_BRANCO_INC"} você irá pagar {"COR_VERDE_INC"}R$%s{"COR_BRANCO_INC"} por salário.", Planos[listitem][Nome], RealStr(Planos[listitem][Valor]));
		return true;
	}
	...
	if ( response )
	{
		switch(listitem)
		{
			case 0: // Limpar ficha criminal
			{
				...
				SendClientMessage(playerid, COR_AZUL, "| INFO | Você removeu {ffffff}%d {"COR_AZUL_INC"}estrelas de procurado e pagou {"COR_LARANJA_INC"}R$%d{"COR_AZUL_INC"}!", GetPlayerWantedLevel(playerid), valor);
		...
		switch(listitem)
		{
			...
			case 1: // limpar pontos na carteira
			{
				...
				ShowPlayerDialog(playerid, MENU_LIMPAR_CARTEIRA, DIALOG_STYLE_MSGBOX, "MENU PREFEITURA » {"COR_BRANCO_INC"}LIMPAR PONTOS NA CARTEIRA", 
					"{"COR_BRANCO_INC"}Você tem {"COR_VERMELHO_INC"}%d{"COR_BRANCO_INC"} pontos em sua habiitação. Para limpar você precisa de {"COR_VERDE_INC"}R$%s{"COR_BRANCO_INC"}.\n\n\t{"COR_ERRO_INC"}Você deseja limpar os pontos da sua Carteira de Habilitação?", "Limpar", "Voltar", call::PLAYER->GetPlayerVarInt(playerid, Multas), RealStr(valor) );
				return true;
			}
		}
		return true;
...
Dialog:MENU_IMPOSTOS(playerid, response, listitem, inputtext[])
{
	...
	OnPlayerEnterDynamicCP(playerid, CP_PREFEITURA);
	return true;
}
Dialog:MENU_LIMPAR_CARTEIRA(playerid, response, listitem, inputtext[])
{
	if ( response )
	{
		if ( Jogador[playerid][Multas] < 1 )
			return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você não tem pontos na carteira de habilitação."), OnPlayerEnterDynamicCP(playerid, CP_PREFEITURA);

		new valor = Jogador[playerid][Multas] * VALOR_POR_MULTA;
		
		if ( GetPlayerMoney(playerid) < valor )
			return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você não tem o dinheiro suficiente."), OnPlayerEnterDynamicCP(playerid, CP_PREFEITURA);
...
Dialog:PAGAR_IMPOSTO_CASA(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if ( Jogador[playerid][PlayerID] != Casa[casaid][Owner][Id] )
			return ShowPlayerDialog(playerid, MENU_IMPOSTOS, DIALOG_STYLE_LIST, "{"COR_DISABLE_INC"}MENU PREFEITURA » {"COR_BRANCO_INC"}PAGAR IMPOSTOS", "Casa\nEmpresa", "Selecionar", "Voltar"), SendClientMessage(playerid, COR_ERRO, "| ERRO | Você não é proprietário desta casa.");
		...
		if ( Casa[casaid][ImpostosPagos] >= 30 )
		{
			...
			SendClientMessage(playerid, COR_AVISO, "CASA: {"COR_BRANCO_INC"}Sua casa subiu para o level {"COR_AZUL_INC"}%d{"COR_BRANCO_INC"}.", Casa[casaid][Level]);
		}
		...
		GivePlayerMoney(playerid, -valor);
		SendClientMessage(playerid, COR_AZUL, "» Você quitou todos os impostos da sua casa no valor de {"COR_VERDE_INC"}R$%s{"COR_BRANCO_INC"}.", RealStr(valor));
		return true;
	}
	ShowPlayerDialog(playerid, MENU_IMPOSTOS, DIALOG_STYLE_LIST, "{"COR_DISABLE_INC"}MENU PREFEITURA » {"COR_BRANCO_INC"}PAGAR IMPOSTOS", "Casa\nEmpresa", "Selecionar", "Voltar");
	return true;
...
Dialog:PAGAR_IMPOSTO_EMPRESA(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if ( strcmp(pName, GetUserName(playerid), true) != 0 || isnull(pName) )
			return ShowPlayerDialog(playerid, MENU_IMPOSTOS, DIALOG_STYLE_LIST, "{"COR_DISABLE_INC"}MENU PREFEITURA » {"COR_BRANCO_INC"}PAGAR IMPOSTOS", "Casa\nEmpresa", "Selecionar", "Voltar"), SendClientMessage(playerid, COR_ERRO, "| ERRO | Você não é proprietário desta empresa.");
		...
		GivePlayerMoney(playerid, -valor);
		SendClientMessage(playerid, COR_AZUL, "» Você quitou todos os impostos da sua empresa no valor de {"COR_VERDE_INC"}R$%s{"COR_BRANCO_INC"}.", RealStr(valor));
		return true;
	}
	ShowPlayerDialog(playerid, MENU_IMPOSTOS, DIALOG_STYLE_LIST, "{"COR_DISABLE_INC"}MENU PREFEITURA » {"COR_BRANCO_INC"}PAGAR IMPOSTOS", "Casa\nEmpresa", "Selecionar", "Voltar");
	return true;
...
#include ../bin/modules/server/maps.pwn
